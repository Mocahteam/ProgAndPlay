...
Call::sp_call Sequence::getFirstCall(){
	...
	if (tr->isSequence())
		return std::dynamic_pointer_cast<Sequence>(tr)->getFirstCall();
	return std::dynamic_pointer_cast<Call>(tr);
}
...
void Sequence::completeIterationDescription()
{
	...
	{
		Sequence::sp_sequence sps = std::dynamic_pointer_cast<Sequence>(getParent().lock());
		unsigned int num = 0;
...
/**
 * \brief Linéarisation de l'ensemble des traces mais seulement pour le premier S. Le vecteur retourné peut contenir des objets de type Sequence qui marquent le début ou la fin d'une séquence. Toutes les traces comprise entre un début et un fin de séquence font parties de la séquence.
 * Exemple, la séquence suivante (C modélisent un Call et S une séquence) :
 *       C
 *      \ /
 *   C C S
 ...
 * \return un vecteur de trace représentant une version linéarisée des traces
 */
std::vector<Trace::sp_trace> Sequence::getLinearSequenceOnlyFirstBoucle(int start, int end)
{
	std::vector<Trace::sp_trace> ret;
	Sequence::sp_sequence sps = shared_from_this();
	start = start < 0 ? 0 : start;
	end = end < 0 || end > (int)sps->size() ? sps->size() : end;
	if (sps->length() > 0)
	{
		...
		tmpSeq->mergeIterationDescription(sps->getIterationDescription());
		tmpSeq->num = tmpSeq->getIterationDescription().rbegin()->first;
		ret.push_back(tmpSeq);
		while (!stack.empty())
		{
			while (!stack.empty() && (sps->isEndReached() || sps->length() == 0))
			{
				stack.pop();
				ret.push_back(std::make_shared<Sequence>("End", -1));
				if (!stack.empty())
					sps = stack.top();
			}
			if (!sps->isEndReached() && sps->length() > 0)
	...
	while(checkFeasibility(windowSize*2, startingPoint)) {
		...
		while (checkFeasibility(windowSize, workingId)) {
			...
			if (!mergeOccurs){
				...
				sps_up = std::make_shared<Sequence>(1);
				...
				if (at(workingId-1)->length() == windowSize){
					...
					sps_up = std::dynamic_pointer_cast<Sequence>(at(workingId-1));
					upStart = workingId-1;
				} else {
			...
			Sequence::sp_sequence sps_down = std::make_shared<Sequence>(1);
			...
			if (at(workingId)->length() == windowSize){
				...
				sps_down = std::dynamic_pointer_cast<Sequence>(at(workingId));
				endDown = workingId;
			} else {
		...
		while (checkFeasibility(minWindowSize, workingId))
		{
			...
			if (!mergeOccurs)
			{
				...
				else
				{
					...
					Sequence::sp_sequence sps_upExtended = std::dynamic_pointer_cast<Sequence>(sps_up->clone());
					while (currentWindowSize < maxLength && bestScore >= 0 && bestScore < 1)
					{
						...
						if (bestScore >= 0)
						{
							...
							if (bestScore == 1 || tmpWorkingId >= (signed)size() || (at(tmpWorkingId)->isEvent() && Trace::inArray(std::dynamic_pointer_cast<Event>(at(tmpWorkingId))->getLabel().c_str(), Event::noConcatEventsArr) > -1))
...
void Sequence::evaluateMerging(int workingId, int maxLength, Sequence::sp_sequence sps_up, bool originalUpSize, float & bestScore, Sequence::sp_sequence & bestMerge, int & endDown){
	...
	for (int i = workingId; i < endPos; i++)
	{
		...
		if (i == workingId && sps_down->at(0)->isSequence())
			sps_res = mergeEquivalentSequences(sps_up, std::dynamic_pointer_cast<Sequence>(sps_down->at(0)), false, &nbAlign, &nbOpt);
		else{
...
Sequence::sp_sequence Sequence::mergeAlignedSequences(Sequence::sp_sequence seq1, Sequence::sp_sequence seq2)
{
	...
	Sequence::sp_sequence sps_merging = std::make_shared<Sequence>(seq1);
	...
	while (i < seq1->size() || j < seq2->size())
	{
		...
		if (i < seq1->size() && j < seq2->size() && !seq1->at(i)->getAligned().expired() && !seq2->at(j)->getAligned().expired())
		{
			...
			else
			{
				...
				sps_merging->addTrace(mergeAlignedSequences(std::dynamic_pointer_cast<Sequence>(seq1->at(i)), std::dynamic_pointer_cast<Sequence>(seq2->at(j))));
			}
			i++;
			j++;
		}
...
Sequence::sp_sequence Sequence::mergeEquivalentSequences(Sequence::sp_sequence sps_up, Sequence::sp_sequence sps_down, bool strict, int* nbAlignRes, int* nbOptRes)
{
	...
	// create an empty new sequence based on up sequences and as many iterations defined in down sequence. This sequence will store merging of up and down sequences
	Sequence::sp_sequence sps_merging = std::make_shared<Sequence>(sps_up);
	...
	// Look for the next trace into the up sequence
	Trace::sp_trace spt_up = sps_up->next();
	// Look for the next trace into the down sequence
	...
	while (!upStack.empty() || !downStack.empty())
	{
		...
		else
		{
			...
			{ // nous sommes ici en non strict (cf "if ((!spt_up || !spt_down) && strict)")
				...
				Sequence::sp_sequence sps_opt = std::make_shared<Sequence>(1, false, true);
			...
			else
			{
				...
				if (spt_up->isSequence())
				{
					...
					sps_up = std::dynamic_pointer_cast<Sequence>(spt_up);
					sps_up->reset();
				}
				...
				if (spt_down->isSequence())
				{
					...
					sps_down = std::dynamic_pointer_cast<Sequence>(spt_down);
					sps_down->reset();
				}
				...
				if (scoreUpDown > scoreNextUpDown && scoreUpDown > scoreUpNextDown)
				{
					...
					else
					{
						...
						if ((!spt_up->isSequence() && spt_down->isSequence()) || len_up < len_down)
						{
							...
							sps_merging = std::make_shared<Sequence>(sps_down);
						...
						else if ((spt_up->isSequence() && !spt_down->isSequence()) || len_up > len_down)
						{
							...
							sps_merging = std::make_shared<Sequence>(sps_up);
						...
						else
						{
							...
							sps_merging = std::make_shared<Sequence>(sps_up, sps_down);
					...
					else if (scoreNextUpDown > scoreUpNextDown || (scoreNextUpDown == scoreUpNextDown && len_up >= len_down))
					{
						...
						if (spt_up->isSequence())
						{
							...
							if (!sps_up->getParent().expired())
								sps_up = std::dynamic_pointer_cast<Sequence>(sps_up->getParent().lock());
						}
						next_up = true;
					}
					...
					else if (scoreUpNextDown > scoreNextUpDown || (scoreUpNextDown == scoreNextUpDown && len_down > len_up))
					{
						...
						if (spt_down->isSequence())
						{
							...
							if (!sps_down->getParent().expired())
								sps_down = std::dynamic_pointer_cast<Sequence>(sps_down->getParent().lock());
						}
						next_down = true;
					}
				}
...
bool Sequence::findAndProcessOptionalTokens(int startingPoint)
{
	...
	for (int currentPos = startingPoint; currentPos < (int)size(); currentPos++)
	{
		...
		{
			Sequence::sp_sequence subseq = std::dynamic_pointer_cast<Sequence>(at(currentPos));
			...
			std::pair<int, Sequence::sp_sequence> contexteRotatedUp(-1, std::dynamic_pointer_cast<Sequence>(context.second->clone()));
			...
			std::pair<int, Sequence::sp_sequence> contexteRotatedDown(-1, std::dynamic_pointer_cast<Sequence>(context.second->clone()));
...
void Sequence::reduceSubsequences()
{
	...
	for (int i = 0 ; i < (signed)size() ; i++)
	{
		if (at(i)->isSequence() && !at(i)->isOptional())
		{
			if (!firstSeq)
			{
				firstSeq = std::dynamic_pointer_cast<Sequence>(at(i));
				seqPos = i;
			}
			else
				return;
	...
	Sequence::sp_sequence prevSeq = std::make_shared<Sequence>(1, false, true);
	Sequence::sp_sequence nextSeq = std::make_shared<Sequence>(1, false, true);
...
std::pair<int, Sequence::sp_sequence> Sequence::extractContext(int currentPos, int *startPos, int *endPos)
{
	...
	if (at(currentPos)->isSequence())
	{
		...
		Sequence::sp_sequence seq = std::dynamic_pointer_cast<Sequence>(at(currentPos));
...
int Sequence::tryToProcessUpRotation(int seqPos)
{
	...
	if (at(seqPos)->isSequence())
	{
		...
		Sequence::sp_sequence seq = std::dynamic_pointer_cast<Sequence>(at(seqPos));
...
int Sequence::processUnaggregateTracesDueToInsertedTokens(int seqPos)
{
	...
	if (at(seqPos)->isSequence())
	{
		...
		if (bestScore >= MIN_SCORE)
		{
			...
			TracesParser::findBestAlignment(seq->getTraces(), upPart->getTraces(), true);
		...
		if (bestScore >= MIN_SCORE)
		{
			...
			TracesParser::findBestAlignment(seq->getTraces(), downPart->getTraces(), true);
			// Et procéder à la fusion
...
int Sequence::processInclusiveSequences(int seqPos)
{
	...
	if (at(seqPos)->isSequence())
	{
		...
		do
		{
			...
			{
				// On garde une ref de cette sequence
				Sequence::sp_sequence seq_candidate = std::dynamic_pointer_cast<Sequence>(at(i));
				// et on détermine son score
		...
		if (bestScore.first / bestScore.second >= MIN_SCORE)
		{
			...
			{
				// Recalculer le meilleur alignement
				TracesParser::findBestAlignment(seq->getTraces(), std::dynamic_pointer_cast<Sequence>(at(bestPos))->getTraces(), true);
				// Effectuer la fusion entre la séquence courrante et la meilleur séquence suivante trouvée
				Sequence::sp_sequence sps_merged = mergeAlignedSequences(seq, std::dynamic_pointer_cast<Sequence>(at(bestPos)));
				// Intégrer la fusion dans la trace
				traces[seqPos] = sps_merged;
...
std::pair<double, double> Sequence::computeBestCorrectedScore(std::vector<Trace::sp_trace> &traces1, std::vector<Trace::sp_trace> &traces2)
{
	...
	std::pair<double, double> res = TracesParser::findBestAlignment(traces1, traces2, true);
...
void Sequence::setIterDesc(std::map<unsigned int, unsigned int> newIterDesc){
	iterDesc = newIterDesc;
}