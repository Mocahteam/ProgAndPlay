...
public:
   ...
   * \param end indice de fin de linéarisation de la séquence (valeur par défaut : -1 => fin de la trace)
	 ...
	 * \return un vecteur de trace représentant une version linéarisée des traces
	 */
	std::vector<Trace::sp_trace> getLinearSequenceOnlyFirstBoucle(int start = 0, int end = -1);
	...
	/**
	 * \brief retourne l'indice dans la trace linéarisée contenant le nième Call. Si num <= 0 retourne l'indice du premier Call de la trace linéarisée. Si num >= au nombre de call dans la trace linéarisée alors l'indice du dernier Call dans la trace linéarisée est retourné. Peut retourner -1 si la strace linéarisée ne contient aucun Call.
	 */
   ...
   * \param end indice de fin dans la trace linéarisée (valeur par défaut : -1 => fin de la trace)
	...
	*/
	...
	/**
	 * \brief Instituer iterDesc
	 * 
	 * Cette fonction est utilisée pour instituer iterDesc
	 * 
